...
router.get('/releases/latest', async (req, res) => {
    try {
        ...
        const toCdn = (url) => (url && typeof url === 'string') ? url.replace('https://f004.backblazeb2.com/file/alpha-studio', cdn) : url;

        const setting = await SystemSetting.findOne({ key: 'crm_latest_release' });
        if (setting && setting.value) {
            const val = { ...setting.value };
            if (val.windowsInstallerUrl) val.windowsInstallerUrl = toCdn(val.windowsInstallerUrl);
            if (val.androidApkUrl) val.androidApkUrl = toCdn(val.androidApkUrl);
            return res.json({
                success: true,
                data: val
            });
        ...
        let b2Data = null;
        try {
            const response = await fetch(`${cdn}/crm-app/version.json`);
            if (response.ok) {
                ...
                b2Data = {
                    ...
                    windowsInstallerUrl: windowsAsset ? toCdn(windowsAsset.browser_download_url) : `${cdn}/crm-app/releases/alpha-crm-v1.0.0.exe`,
                    androidApkUrl: androidAsset ? toCdn(androidAsset.browser_download_url) : `${cdn}/crm-app/releases/alpha-crm-v1.0.0.apk`,
        ...
        const latestRelease = b2Data || {
            version: '1.0.0',
            windowsInstallerUrl: `${cdn}/crm-app/releases/alpha-crm-v1.0.0.exe`,
            androidApkUrl: `${cdn}/crm-app/releases/alpha-crm-v1.0.0.apk`,
